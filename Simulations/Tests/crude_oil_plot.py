...
with open(f'../Tests/Instances/crude_oil_mindets_threat_different_gamma.pkl', 'rb') as inp:
    mindets = pickle.load(inp)
    ...
    mindets_df = pd.DataFrame({"Gamma": [m[0] for m in mindets],
                                "Costs": [m[2].mindet_history.get_cost() for m in mindets],
                                "Errors": [m[2].algs_histories[1].get_purchase_error(off) for m in mindets]})

    N = 8
    colors = plt.cm.Purples(np.linspace(0, 1, N))
    i = 2

    for gamma in mindets_df["Gamma"].drop_duplicates():

        # if gamma in [.1, .4, .7, .9]:
        mindets_gamma_df = mindets_df[mindets_df["Gamma"] == gamma].sort_values(by=['Errors'])

        ax.plot(mindets_gamma_df['Errors'], mindets_gamma_df['Costs'],
                label=f"$ \gamma = {gamma}$", color=colors[i])

        i += 1

    x = mindets_gamma_df['Errors']
