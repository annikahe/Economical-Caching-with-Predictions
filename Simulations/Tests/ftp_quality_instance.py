...
for ti in treatments_input:
    for tp in treatments_predictions:
        ...
        for i in range(num_generations):
            ...
            if tp == "unif":
                e1, e2, r = quality_of_FtP(prices, demands, num_repetitions * 10, prediction_type=tp)
            else:
                e1, e2, r = quality_of_FtP(prices, demands, num_repetitions, prediction_type=tp)

            eta1.extend(e1)
            eta2.extend(e2)
            ratios.extend(r)

        ph.save_objects([eta1, eta2, ratios], f'Instances/FtP_quality_input-{ti}_preds-{tp}.pkl')
